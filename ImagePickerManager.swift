...
class ImagePickerManager: NSObject {
  ...
  func configureAndShowAlerts() {
      let pickerAlert = UIAlertController(title: "Choose image from", message: nil, preferredStyle: .actionSheet)
    if pickerTypes.contains(.camera) {
        let cameraAction = UIAlertAction(title: "Profile camera", style: .default) { UIAlertAction in
        self.openCamera()
      }
